...
async function openDetail(id){
  ...
  try{
    ...
    $('detailBody').innerHTML=`<div><div class="gallery-main">${images[0]?`<img id="mainImage" src="${escapeHtml(images[0])}" alt="Ảnh nhà">`:''}<button type="button" class="gallery-share-badge" id="shareDetail" title="Chia sẻ căn nhà này"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg><span>Chia sẻ</span></button></div><div class="thumbs">${images.map((src,index)=>`<img class="${index===0?'active':''}" src="${escapeHtml(src)}" alt="Ảnh ${index+1}">`).join('')}</div></div><div><section class="content-panel"><h3>Nội dung nhà</h3><p>${escapeHtml(p.raw_text||p.notes||'Chưa có nội dung mô tả.')}</p></section><section class="info-panel"><div class="price">${escapeHtml(p.price_text||'Liên hệ')}</div><h2>${escapeHtml(p.address||p.property_id)}</h2><div class="meta">${escapeHtml([p.street,p.ward,p.district].filter(Boolean).join(' · '))}</div><div class="info-grid">${[['Diện tích',p.area_text],['Kích thước',p.dimensions],['Phòng ngủ',p.bedrooms],['Phòng tắm',p.bathrooms],['Kết cấu',p.structure],['Pháp lý',p.legal],['Liên hệ',p.phone],['Loại BĐS',p.property_type]].map(([label,value])=>`<div><small>${label}</small><strong>${escapeHtml(value||'—')}</strong></div>`).join('')}</div></section>${state.adminUnlocked?adminToolsHtml(p):''}<section class="direct-contact"><div><span>Liên hệ trực tiếp</span><strong>${escapeHtml(p.phone||'Chưa có số điện thoại')}</strong></div>${p.phone?`<a href="tel:${escapeHtml(phoneHref(p.phone))}" aria-label="Gọi ${escapeHtml(p.phone)}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7.2 3.5 9.6 7c.35.5.28 1.18-.16 1.62l-1.3 1.3a14.5 14.5 0 0 0 5.94 5.94l1.3-1.3c.44-.44 1.12-.51 1.62-.16l3.5 2.4c.55.38.72 1.11.39 1.69l-1 1.75c-.34.59-.98.95-1.66.93C10.1 20.95 3.05 13.9 2.83 5.77c-.02-.68.34-1.32.93-1.66l1.75-1c.58-.33 1.31-.16 1.69.39Z"/></svg>Gọi ngay</a>`:''}</section></div>`;
