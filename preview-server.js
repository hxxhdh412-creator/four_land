...
http.createServer(async (req,res)=>{
  ...
  if(url.pathname==="/api/admin-login") {
    ...
    let raw="";req.on("data",chunk=>raw+=chunk);req.on("end",()=>{try{const body=JSON.parse(raw||"{}");if(String(body.code||"")!==adminCode)return send(res,401,{ok:false,error:"Mã truy cập không đúng"});res.setHeader("Set-Cookie",`fourland_admin=${previewAdminToken}; Path=/; HttpOnly; SameSite=Strict; Max-Age=28800`);return send(res,200,{ok:true,authenticated:true})}catch{return send(res,400,{ok:false,error:"Dữ liệu không hợp lệ"})}});return;
  }
  ...
  if(url.pathname==="/api/facets") {if(databaseEnabled){try{const result=await dbRequest("properties?select=district,ward,street,property_type&limit=10000"),values=key=>[...new Set(result.data.map(item=>item[key]).filter(Boolean))].sort((a,b)=>String(a).localeCompare(String(b),"vi"));return send(res,200,{ok:true,districts:values("district"),wards:values("ward"),streets:values("street"),types:values("property_type")})}catch(error){return send(res,500,{ok:false,error:error.message})}}return send(res,200,{ok:true,districts:[...new Set(rows.map(x=>x.district))],wards:[...new Set(rows.map(x=>x.ward))],streets:[...new Set(rows.map(x=>x.street))],types:[...new Set(rows.map(x=>x.property_type))]});}
  if(url.pathname==="/api/properties") {
    ...
    const filtered=rows.filter(row=>(!q||JSON.stringify(row).toLowerCase().includes(q))&&["district","ward","street"].every(key=>!url.searchParams.get(key)||row[key]===url.searchParams.get(key))&&(!url.searchParams.get("type")||row.property_type===url.searchParams.get("type")));
    return send(res,200,{ok:true,rows:filtered,total:filtered.length,page:1,pageSize:24});
  }
  if(url.pathname==="/api/property") {if(databaseEnabled){try{const result=await dbRequest(`properties?select=*,property_images(*)&property_id=eq.${encodeURIComponent(url.searchParams.get("id")||"")}&limit=1`),property=result.data[0];return send(res,property?200:404,property?{ok:true,property}:{ok:false,error:"Không tìm thấy hồ sơ"})}catch(error){return send(res,500,{ok:false,error:error.message})}}const row=rows.find(item=>item.property_id===url.searchParams.get("id"));return send(res,row?200:404,row?{ok:true,property:row}:{ok:false,error:"Không tìm thấy hồ sơ"});}
