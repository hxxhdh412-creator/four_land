...
function formatPublicAddress(property) {
  ...
  return loc ? ('Bất động sản ' + loc) : (value(property.property_id) || 'Bất động sản TP.HCM');
}
...
function maskDescriptionText(text, address, street) {
  ...
  if (address) {
    const rawAddr = value(address);
    const streetName = stripHouseNumber(rawAddr) || value(street);
...
function propertyPath(property) {
  const label = formatPublicAddress(property);
...
function renderPropertyPage(property) {
  ...
  const titleBase = formatPublicAddress(property);
  ...
  const rawDesc = maskDescriptionText(property.raw_text || property.notes || `${property.property_type || "Bất động sản"} tại ${titleBase}, ${location}`, property.address, property.street);
  const description = value(rawDesc).slice(0, 158);
